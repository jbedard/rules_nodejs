...
function runterser(inputFile, outputFile) {
  // Must stay in sync with the declared output file in the terser.bzl _run_terser()
  ...
  const args = [
    require.resolve('build_bazel_rules_nodejs_terser_deps/node_modules/terser/bin/uglifyjs'),
...
const isDirectory = fs.lstatSync(fs.realpathSync(path.join(process.cwd(), input))).isDirectory();

if (!isDirectory) {
  runterser(input, output);
} else {
  ...
  dir.forEach(f => {
    if (f.endsWith('.js')) {
      ...
      runterser(inputFile, outputFile);
    }
  });
}
